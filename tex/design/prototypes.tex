...
For the second interview with Kristine Niss-Henriksen, an HTML prototype was made from the primitive prototypes. In the Figure \vref{fig:contactbook} are two screenshots from the contact book, where the right side of the figure is a light box with a post from the contact book. Much like a calendar, the contact book's posts are accessed through clicking on a date. When clicked, a light box pops up that contains the message and pictures. If the user has not seen the message before, the signal ``Ny!'' in the right side should be visible. The user is able to fold and unfold the posts for each date, to see or hide the posts in the contact book. 
