...
The advantage of using a unit test framework is that the developer does not have to program the entire unit test, which would be very time-consuming. Unit test frameworks help developers to prepare a test more quickly, run the test and review the result more easily\cite{UTF}.  

Several unit testing frameworks exist for various programming languages and development platforms. The frameworks: CppUnit for C++, JUnit for Java, NUnit for .NET, are known as xUnit. Among the xUnits\cite{UTF}, is SimpleTest PHP coding language  also a unit testing framework.
